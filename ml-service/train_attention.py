"""
Model 4: CNN with Self-Attention on mel spectrogram features.

Approach: A CNN extracts local features from the mel spectrogram, then a
multi-head self-attention mechanism captures global dependencies across
time frames. This lets the model learn WHICH parts of the audio are most
informative for depression detection (e.g., pauses, pitch drops, monotone).
...
BATCH_SIZE = 32
EPOCHS = 100
LEARNING_RATE = 0.0005  # slightly lower for attention stability
DROPOUT_RATE = 0.3
...
def create_cnn_attention(input_shape=(128, 313, 1), dropout_rate=0.3) -> keras.Model:
    ...
    1. Two CNN blocks reduce mel spectrogram spatial dimensions.
    2. Feature maps are reshaped into a sequence (time, features).
    3. Multi-head attention captures global temporal relationships.
    4. Classifier uses the attended representation.
    """
    ...
    # Reshape to sequence: (32, 78, 64) -> (78, 32*64) = (78, 2048)
    ...
    model = keras.Model(inputs, outputs, name="cnn_attention")
    return model
...
def create_combined_tflite(classification_model):
    ...
    return keras.Model(inputs=audio_input, outputs=output, name="attention_depression_detector")
...
def main():
    print("=" * 60)
    print("  Model 4: CNN + Self-Attention on Mel Spectrograms")
    print("=" * 60)
    ...
    # 2. Extract mel features — (N, 128, 313, 1)
    ...
    # 4. Build & train
    print("\n[4/6] Building CNN + Self-Attention model...")
    model = create_cnn_attention(input_shape=X_train.shape[1:], dropout_rate=DROPOUT_RATE)
    model.compile(
    ...
    best_model_path = os.path.join(MODEL_DIR, f"attention_{TIMESTAMP}_best.keras")
    callbacks = [
    ...
    final_path = os.path.join(MODEL_DIR, f"attention_{TIMESTAMP}_final.keras")
    ...
    combined.summary()
    tflite_path = os.path.join(MODEL_DIR, "attention_depression_combined.tflite")
    ...
    print("\n" + "=" * 60)
    print("  DONE! CNN + Attention Training Complete")
    print("=" * 60)
