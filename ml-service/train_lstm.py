"""
Model 1: Bidirectional LSTM on mel spectrogram features.

Approach: Treat mel spectrogram as a sequence of time frames, each with 128 mel bands.
The LSTM captures long-range temporal dependencies in speech patterns.
...
# ============================================================
# Configuration (same audio pipeline as mel CNN)
# ============================================================
...
def create_bilstm(input_shape=(313, 128), dropout_rate=0.3) -> keras.Model:
    ...
    Two stacked BiLSTM layers capture forward/backward temporal patterns.
    """
    model = keras.Sequential(
        [
            layers.Input(shape=input_shape),
            # BiLSTM Layer 1: captures local temporal patterns
            layers.Bidirectional(
                layers.LSTM(64, return_sequences=True, dropout=dropout_rate)
            ),
            layers.BatchNormalization(),
            # BiLSTM Layer 2: captures global temporal patterns
            layers.Bidirectional(
...
# ============================================================
# Shared helpers (feature extraction, evaluation, TFLite)
# ============================================================
def extract_mel_tf(audio_np, mel_weights):
    """Extract mel spectrogram using tf.signal (matching TFLite pipeline)."""
    ...
    return mel_db.numpy()  # shape: (time_steps, n_mels)
...
def create_combined_tflite(classification_model):
    """Combined model: raw audio -> mel spectrogram -> LSTM -> prediction."""
    ...
    # LSTM expects (time, n_mels) - no transpose needed, STFT gives (time, freq)
    x = x[:, :EXPECTED_TIME_STEPS, :]
    pad_size = EXPECTED_TIME_STEPS - tf.shape(x)[1]
    paddings = tf.stack([
        tf.constant([0, 0]),
    ...
    return keras.Model(inputs=audio_input, outputs=output, name="lstm_depression_detector")
...
def main():
    print("=" * 60)
    print("  Model 1: Bidirectional LSTM on Mel Spectrograms")
    print("=" * 60)
    ...
    # 2. Extract features — shape: (N, time_steps, n_mels) for LSTM
    ...
    for i, audio in enumerate(audio_list):
        mel = extract_mel_tf(audio, mel_weights)  # (time, n_mels)
        if mel.shape[0] < EXPECTED_TIME_STEPS:
            mel = np.pad(mel, ((0, EXPECTED_TIME_STEPS - mel.shape[0]), (0, 0)))
        else:
    ...
    X = np.array(features)  # (N, 313, 128) — sequence format
    y = np.array(labels)
    ...
    # 4. Build & train
    print("\n[4/6] Building Bidirectional LSTM...")
    model = create_bilstm(input_shape=X_train.shape[1:], dropout_rate=DROPOUT_RATE)
    model.compile(
    ...
    best_model_path = os.path.join(MODEL_DIR, f"lstm_{TIMESTAMP}_best.keras")
    callbacks = [
    ...
    final_path = os.path.join(MODEL_DIR, f"lstm_{TIMESTAMP}_final.keras")
    ...
    combined.summary()
    tflite_path = os.path.join(MODEL_DIR, "lstm_depression_combined.tflite")
    ...
    print("\n" + "=" * 60)
    print("  DONE! BiLSTM Training Complete")
    print("=" * 60)
